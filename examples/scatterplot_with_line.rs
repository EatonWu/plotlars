...
fn main() {
    ...
    let sp = ScatterPlot::builder()
        ...
        .x_axis(&axis.clone().value_range(vec![0.0, 6500.0]))
        .y_axis(&axis.clone().value_range(vec![170.0, 240.0]))
        .legend(&Legend::new().x(0.85).y(0.15))
        .build();

    let mut plotly_plot = sp.to_plotly();

    // add some random line to demonstrate
    let scatter = plotly::Scatter::new(vec![0, 6500], vec![0, 240]);
    plotly_plot.add_trace(scatter);
    plotly_plot.show();
}
